...
internal class SettingsHomePresenter(
        ...
        val dock = ModernNavigationBar(
            ...
            backgroundFactory = { surface, radius ->
                when (surface) {
                    ...
                    ModernNavigationSurface.SELECTION -> activity.skinSelectionBackground(activity.monetColors.surface, radius)
                }
            },
    ...
    private fun applyScrollInsets() {
        val top = if (headerInset > 0) headerInset else dp(8)
    ...
    internal val currentHeaderInset: Int get() = headerInset
